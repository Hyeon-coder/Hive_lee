	...
	if grep -q "ERROR SUMMARY: [^0]" "${temp_file}"; then
		...
		if ((timeout == 124)); then
			...
			if (( ${SHOW_VALGRIND} == 1 )); then
				...
				printf "Valgrind found an error. To get valgrind output, run with --show-valgrind or run manually\n" >> last_err_log.txt
			fi
		fi
	else
